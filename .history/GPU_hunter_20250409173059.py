    ...
    try:
        output = subprocess.check_output(
            ["nvidia-smi", "--query-gpu=memory.used,memory.total",
             "--format=csv,noheader,nounits", "-i", "1,3,5,6"],
...
def main():
    check_interval = 60*5  # 检查间隔（秒）
    command_to_run = "nohup bash /data/wuxinrui/LLaMA-Factory/TCM_add_train_long_short/train.sh &"  # 替换为需要执行的命令
