...
class RayPPOTrainer:
    ...
    def fit(self):
        ...
        for current_epoch in tqdm(range(now_epoch, self.config.trainer.total_episodes), desc="Episode", position=0):
            current_epoch_copy = current_epoch + 1 #g 从0开始 -> 从1开始
            self._create_dataloader(current_epoch_copy)
            for batch_dict in tqdm(itertools.islice(self.train_dataloader, new_step_in_now_epoch, None), desc="Running step", position=1):
                self.global_step += 1
