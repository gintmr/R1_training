...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto) -> DataProto:
        ...
        if self.stage == "2":
            ...
            with self.update_sampling_params(**prompts.meta_info):
                default_sampling_params = SamplingParams(n=1, max_tokens=6800, temperature=1.0)
                ...
                final_response_ids = VF.pad_2d_list_to_length(
                    final_response_ids, self.pad_token_id, max_length=self.config.response_length
                ).to(input_ids.device)
